...
const InputField = ({ title, isEmptyAllow = true, titleColor = "#000", placeholder = 'Enter Input', checkbox=false }) => {
  return (
    <div className='flex flex-col gap-[8px]'>
        <div className='flex gap-2 items-center'>
          <span className={`font-semibold text-[16px] leading-[24px] text-[${titleColor}]`}>{title}</span>
          {
            !isEmptyAllow 
            &&
          ...
          {
            checkbox && 
          <input type="checkbox" class="w-5 h-5 accent-[#F08921]" />
          }
        </div>
        <input className='outline-none w-full h-[40px] rounded-lg border border-[#7F7F7F] flex items-center px-4 placeholder:font-normal placeholder:text-[14px] placeholder:leading-[20px] placeholder:text-[#A9A9A9]' placeholder={placeholder}/>
    </div>
  )
}
